...
API_HOME = 'http://myapi:3000/entity/list_details/'
 
# Scenarios
 
scenarios('../features/getentitylistnew.feature')
...
#@given(parsers.parse('the requestor Id is "{requestor_id}" with role as "{requestor_role}" for an entity_id "{entity_id}" for smoke testing'), target_fixture = 'smoke_get_request')
@given(parsers.parse('the requestor Id is "{requestor_id}" with role as "{requestor_role}" for an entity_id "{entity_id}" for smoke testing'),target_fixture = 'smoke_get_request')
def smoke_get_request(requestor_id, requestor_role, entity_id):
...
@when(parsers.parse('a GET request for an endpoint /entity/list_details/ is triggered to get details of an entity list'), target_fixture='get_request')
def get_request():
    pass
...
@then(parsers.parse('the result should return following details of entity with its name "{name}" "{category}" "{entity_id}" "{phone}" "{email}" and "{website}"'))
def unit_get_response_assert(unit_get_request, name, category, entity_id,phone, email, website):
    ...
    assert assert_response[0][0]['details']['category'] == 'hospital'   
