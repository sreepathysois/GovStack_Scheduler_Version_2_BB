...
API_HOME = 'http://myapi:3000/event/list_details/'
 
# Scenarios
 
scenarios('../features/geteventlistnew.feature')
...
@given(parsers.parse('the requestor Id is "{requestor_id}" with role as "{requestor_role}" for an event_id "{event_id}" for smoke testing'),target_fixture = 'smoke_get_request')
def smoke_get_request(requestor_id, requestor_role, event_id):
...
@when(parsers.parse('a GET request for an endpoint /event/list_details/ is triggered to get details of an event list'), target_fixture='get_request')
def get_request():
    pass
...
def unit_get_response_assert(unit_get_request, name, category, event_id,host_entity_id, venue):
    ...
    assert assert_response[0][0]['details']['category'] == 'doctor_consultation'   
